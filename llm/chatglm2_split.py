...
def main():
    ...
    print("-----------------------origin-----------------------")
    query = torch.matmul(inputs, q_weight)
    key = torch.matmul(inputs, k_weight)
    value = torch.matmul(inputs, v_weight)
    qkv, present_v, present_k = attention(query, key, value, o_weight, rope_cache, 
    ...
    for i in range(num_devices):
        ...
        q_weight_0 = torch.cat([q_weight[:, i*2048//num_devices:(i+1)*2048//num_devices], q_weight[:,i*2048//num_devices+2048:(i+1)*2048//num_devices+2048]], dim=1)
        k_weight_0 = k_weight
        v_weight_0 = v_weight
        o_weight_0 = torch.cat([o_weight[i*2048//num_devices:(i+1)*2048//num_devices], o_weight[i*2048//num_devices+2048:(i+1)*2048//num_devices+2048]], dim=0)

        query_0 = torch.matmul(inputs, q_weight_0)
        key_0 = torch.matmul(inputs, k_weight_0)
        value_0 = torch.matmul(inputs, v_weight_0)
        history_k_0 = history_k
        history_v_0 = history_v

        qkv_0, present_v_0, present_k_0 = attention(query_0, key_0, value_0, o_weight_0, rope_cache, 
